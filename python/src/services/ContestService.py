...
class ContestService:
    ...
    def getContests(self, level: Level) -> List[Contest]:
        if level != None:
            ...
            return list(self._contestRepository.findAll())
    ...
    def createContestant(self,contest_id : int, user_name: str ) -> Contestant:
        contest_creator = self._contestRepository.findById(contest_id)
        if contest_creator is None:
            raise Exception(f"Contest with id: {contest_id} not found")

        user_creator = self._userRepository.findByName(user_name)
        if user_creator is None:
            raise Exception(f"Username: {user_name} not found")
        
        contest_status = contest_creator.get_status()
        if contest_status == ContestStatus.ENDED:
            raise Exception("Cannot create contestant for ended contest")

        c = Contestant(user_creator, contest_creator)

        saved_c = self._contestantRepository.save(c)
    ...
    def contestHistory(self,contest_id) -> List[Contestant]:
        ...
        constestant_list = self._contestantRepository.findAllByContestId(contest_id)
        return sorted(constestant_list, key = lambda x: x.get_current_contest_points(), reverse=True)
